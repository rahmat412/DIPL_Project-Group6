            ...
            <form>
              ...
              <div class="relative w-full mb-3">
                ...
                <input
                  ...
                  v-model="state.email"
                />
                <span v-if="v$.name.$error" class="text-left block text-sm px-2 text-red-500">{{v$.email.$errors[0].$message}}</span>
              </div>
...
export default {  
  ...
  methods: {
    ...
    async addOwner() {
      ...
      try {
        await axios.post("http://localhost:5000/owner", {
          ownerID: Math.random().toString(36).substring(2),
          ownerName: this.state.name,
          ownerEmail: this.state.email,
          ownerPassword: this.state.password,
          ownerPhone: this.state.phone,
        });
        this.sAlert("success","Yeay..","Adding Owner Success");
        this.close();
      } catch (err) {
        console.log(err);
        this.sAlert("error","Oops..","Adding Owner Failed");
      }
    },
