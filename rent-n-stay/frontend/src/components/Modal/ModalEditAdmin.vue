...
export default {
  ...
  methods: {
    async getAdmin() {
      try {
        await axios.get(
          'http://localhost:5000/admin/'+this.id
        ).then((response) => {
          this.state.id = response.data.adminID;
          this.state.name = response.data.adminName;
          this.state.email = response.data.adminEmail;
          this.state.password = response.data.adminPassword;
          this.state.phone = response.data.adminPhone;
        })
      } catch (err) {
        console.log(err);
      }
    ...
    async editAdmin() {
      ...
      try {
        await axios.put('http://localhost:5000/admin/'+this.id,
          {
        ...
        this.sAlert("success","Yeay..","Editing Admin Success");
        this.close();
      } catch (err) {
        console.log(err);
        this.sAlert("error","Oops..","Editing Admin Failed");
      }
    },
